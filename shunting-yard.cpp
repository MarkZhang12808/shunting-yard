...
void postToPrefix(stack* second, stack* stack);
...
void postToPrefix(stack* prefix, stack* postfix){
    node* temp = postfix->peek();
    while(temp != NULL){
        if(temp->getValue() == '+', '-', '*', '/', 'x'){
            node* n3 = new node(temp->getValue());
            prefix->push(n3);
        }
        else if(isdigit(temp->getValue())){
